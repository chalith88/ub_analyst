...
exports.clean = clean;
exports.normalizeAwpr = normalizeAwpr;
exports.decideType = decideType;
exports.expandTenureYears = expandTenureYears;
exports.fanOutByYears = fanOutByYears;
function clean(s) {
    return (s ?? "").replace(/\s+/g, " ").trim();
}
// Normalize any AWPR/AWPLR mention to "AWPR + X%"
function normalizeAwpr(cell) {
    const t = clean(cell);
    ...
    if (m)
        return `AWPR + ${Number(m[1]).toString()}%`;
    // leave as-is if weird
    return "AWPR + ";
}
// Decide Fixed / Floating / Fixed & Floating
function decideType(rateWithSalary, rateWithoutSalary, heading = "") {
    ...
    if (hasAwpr && saysFixed)
        return "Fixed & Floating";
    if (hasAwpr || saysFloating)
        return "Floating";
    return "Fixed";
}
// Expand a tenure label into individual years
function expandTenureYears(label) {
    ...
    if (slash)
        return [Number(slash[1]), Number(slash[2]), Number(slash[3])];
    // "3 Years" or "1 Year"
...
function fanOutByYears(base, years) {
    if (!years.length)
        return [{ ...base, tenureYears: undefined }];
    return years.map((y) => ({ ...base, tenureYears: y }));
}
