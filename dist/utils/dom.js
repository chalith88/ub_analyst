...
exports.acceptAnyCookie = acceptAnyCookie;
exports.clickLeftMenu = clickLeftMenu;
async function acceptAnyCookie(page) {
    const buttons = [
        "button:has-text(\"OK\")",
        "button:has-text(\"Ok\")",
...
async function clickLeftMenu(page, label) {
    const nav = page.locator("nav.grid").first();
    await nav.waitFor({ state: "visible", timeout: 5000 });
    // try visible role=link first
    let link = nav.getByRole("link", { name: new RegExp(`^${escapeRegex(label)}$`, "i") }).first();
