...
exports.scrapeSampath_OCR = scrapeSampath_OCR;
const path_1 = __importDefault(require("path"));
const promises_1 = __importDefault(require("fs/promises"));
const child_process_1 = require("child_process");
function run(cmd, args, cwd) {
    return new Promise((resolve, reject) => {
        const p = (0, child_process_1.spawn)(cmd, args, { cwd, stdio: "inherit", shell: process.platform === "win32" });
...
async function ocrToLines(pdfPath, workDir) {
    await ensureDir(workDir);
    const base = path_1.default.join(workDir, "sampath");
    const { code: ppmCode } = await run("pdftoppm", ["-png", "-r", "300", pdfPath, base]);
    if (ppmCode !== 0)
        throw new Error("pdftoppm failed");
    const files = (await promises_1.default.readdir(workDir)).filter(f => /^sampath-\d+\.png$/i.test(f)).sort();
    if (files.length === 0)
        throw new Error("No PNG pages produced");
    const words = [];
    for (const f of files) {
        const png = path_1.default.join(workDir, f);
        ...
        const tsv = await promises_1.default.readFile(tsvPath, "utf8");
    ...
    for (const w of words) {
        if (!bucket.length || Math.abs(bucket[0].y - w.y) <= Y_TOL)
            bucket.push(w);
        else {
            flush();
            bucket = [w];
...
function extractSevanaFixed(lines, source, debug) {
    const out = [];
    ...
    for (let i = sevanaHeaderIdx + 1; i < lines.length; i++) {
        ...
        if (/All\s*Other\s*Consumption|Overdraft|Leasing|Pawning/i.test(t)) {
            end = i;
            break;
        }
    }
    ...
    const bands = [
        { label: "1-3 Years", yrs: [1, 3] },
        { label: "4-5 Years", yrs: [4, 5] },
        { label: "6-7 Years", yrs: [6, 7] },
        { label: "8-10 Years", yrs: [8, 10] },
    ];
    let foundAny = false;
...
function extractSevanaFloating(lines, source, debug) {
    const out = [];
...
function extractLabeledProduct(lines, source, labelRe, product, tenureMax, notes, debug) {
    const out = [];
    const idx = findIdx(lines, labelRe);
    if (idx === -1) {
        debug.push({ section: notes, reason: "heading not found" });
        return out;
    }
    const txt = joinText(lines, idx, Math.min(lines.length, idx + 25));
    const p = firstPercent(txt);
    if (!p) {
        debug.push({ section: notes, reason: "no % found near heading" });
        return out;
    }
...
/** ---------- MAIN ---------- */
async function scrapeSampath_OCR(pdfUrl, outputPath) {
    const workDir = path_1.default.join(process.cwd(), ".work_sampath");
    await ensureDir(workDir);
    // Download
    ...
    const pdfPath = path_1.default.join(workDir, "sampath.pdf");
    await promises_1.default.writeFile(pdfPath, arr);
    // OCR to structured lines
    const lines = await ocrToLines(pdfPath, workDir);
    // Build output (only from what’s actually found)
    const debug = [];
    const out = [];
    out.push(...extractSevanaFixed(lines, pdfUrl, debug), ...extractSevanaFloating(lines, pdfUrl, debug), ...extractAOCL(lines, pdfUrl, debug), 
    // Strict-parse the other labeled products (no hardcode; will skip if not found)
    ...extractLabeledProduct(lines, pdfUrl, /Loan\s*Scheme\s*for\s*Medical\s*Officers/i, "Personal Loan", 7, "Loan Scheme for Medical Officers — applies up to 7 years", debug), ...extractLabeledProduct(lines, pdfUrl, /Personal\s*Loans\s*for\s*Professionals/i, "Personal Loan", 7, "Personal Loans for Professionals — applies up to 7 years", debug), ...extractLabeledProduct(lines, pdfUrl, /Study\s*Smart\s*Education\s*Loan/i, "Education Loan", 8, "Study Smart Education Loan — applies up to 8 years", debug));
    // Write results
    await ensureDir(path_1.default.dirname(outputPath));
    await promises_1.default.writeFile(outputPath, JSON.stringify(out, null, 2), "utf8");
    // Write debug (what was missing / why)
    await promises_1.default.writeFile(path_1.default.join(workDir, "_debug.json"), JSON.stringify({ missing: debug, sampleLines: lines.slice(0, 60) }, null, 2), "utf8");
    console.log(`✅ Sampath OCR finished.
  Output: ${outputPath}
  Items: ${out.length}
  Debug: ${path_1.default.join(workDir, "_debug.json")}
`);
}
//# sourceMappingURL=sampath_ocr.js.map