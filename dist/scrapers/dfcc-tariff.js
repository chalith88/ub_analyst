...
exports.scrapeDfccTariff = scrapeDfccTariff;
const fs_1 = __importDefault(require("fs"));
const node_fetch_1 = __importDefault(require("node-fetch"));
const pdf_parse_1 = __importDefault(require("pdf-parse"));
/* ---------------- Constants ---------------- */
const PDF_URL = "https://s3.ap-southeast-1.amazonaws.com/dfcc.lk/wp-content/uploads/2025/05/29072325/DFCC-Bank-PLC-Tariff-2025-Version-3.1-1.pdf";
...
function writeDebugLines(lines) {
    ensureDir("./work_dfcc");
    fs_1.default.writeFileSync("./work_dfcc/dfcc-tariff-ocr-lines.txt", lines.map((l, i) => `[${i}] ${l}`).join("\n"), "utf8");
}
...
/* ---------------- Home Loan (unchanged) ---------------- */
function parseHomeLoanProcessing(lines, source) {
    const out = [];
    const seen = new Set();
    const headerIdx = lines.findIndex((l) => /5\.1\.3\s*Housing\s*Loan\s*All\s*Categories/i.test(l));
    if (headerIdx < 0)
        return out;
    const slabLines = [];
    ...
    for (const ln of slabLines) {
        ...
        if (!got.description || !got.amount)
            continue;
        const ok = /^Up to Rs\.?\s*[\d,]+$/i.test(got.description) ||
...
function parsePersonalLoanProcessing(lines, source) {
    const out = [];
    const seen = new Set();
    ...
    for (const ln of lines) {
        if (/^5\.2\.1\.1\.[1-5]/.test(ln)) {
            ...
            if (/^N\/A$/i.test(ln) || /^fee$/i.test(ln) || /Equivalent FCY value to LKR/i.test(ln))
                continue;
            slabLines.push(ln);
        }
    }
    // You want all 5 (1 to 5). No dupes!
    const slabNums = new Set();
    for (const ln of slabLines) {
...
/* ---------------- Education Loan ---------------- */
function parseEducationLoanSlabs(lines, source) {
    const out = [];
    const sectionIdx = lines.findIndex((l) => l.trim().startsWith("5.2.1.5Education Loan"));
    if (sectionIdx < 0)
        return out;
    ...
    for (let i = 1; i <= 5; ++i) {
        ...
        const feeRe = /\d{1,3}(?:,\d{3})+/g;
        let m;
        while ((m = feeRe.exec(descPart)) !== null)
            lastFee = m;
        if (!lastFee)
            continue;
        ...
        description = description
            ...
            .replace(/^\.?/, "")
            .trim();
        if (/Over\s*Rs\./i.test(description))
            description = "Over Rs.50 Mn";
        out.push({
            bank: "DFCC Bank",
...
function parseSpecialLoanSlabs(lines, source, section, note) {
    const out = [];
    // Ex: 5.2.1.6One Loan → prefix = 5.2.1.6.
    const sectionIdx = lines.findIndex((l) => l.trim().toLowerCase().startsWith(`5.2.1.${section.toLowerCase()}`));
    if (sectionIdx < 0)
        return out;
...
function parseLegalFeesTitleReport(lines, source) {
    const out = [];
    for (const ln of lines) {
        ...
        let description = "";
        if (slabNo === "1")
            description = "Up to LKR 3,000,000";
        if (slabNo === "3")
            description = "LKR 3,000,001 - 25,000,000";
        if (slabNo === "4")
            description = "Over Rs.25 Mn";
...
function parseLegalFeesMortgageBond(lines, source) {
    const out = [];
    // Find start of Mortgage Bond section
    const startIdx = lines.findIndex(l => /^5\.5\.2\b/.test(l.trim()));
    if (startIdx < 0)
        return out;
    ...
    for (let i = 0; i < descSlabs.length; i++) {
        ...
        if (i === 0) {
            ...
            if (!percLine.includes("%"))
                percLine = lines[idx + 3]?.trim() || "";
            if (!minLine.match(/minimum/i))
                minLine = lines[idx + 4]?.trim() || "";
            // Add "0" if missing
...
function parseLegalFeesPersonalCorporateGuarantees(lines, source) {
    const out = [];
    const startIdx = lines.findIndex(l => l.trim().startsWith("5.7"));
    if (startIdx < 0)
        return out;
...
function parseLegalFeesOtherCharges(lines, source) {
    const out = [];
...
/* ---------------- Combined Parser ---------------- */
function parseDfccTariffLines(lines, source) {
    const out = [];
...
/* ---------------- Main Scraper ---------------- */
async function scrapeDfccTariff() {
...
exports.default = scrapeDfccTariff;
/* ---------------- Local Test ---------------- */
if (require.main === module) {
    (async () => {
        const rows = await scrapeDfccTariff();
        ensureDir("./output");
        fs_1.default.writeFileSync("./output/dfcc-tariff.json", JSON.stringify(rows, null, 2));
...
//# sourceMappingURL=dfcc-tariff.js.map