...
exports.scrapeSampath = scrapeSampath;
const promises_1 = __importDefault(require("fs/promises"));
const pdf_parse_1 = __importDefault(require("pdf-parse"));
/* ------------------------- utils ------------------------- */
const SOURCE = "https://www.sampath.lk/common/loan/interest-rates-loan-and-advances.pdf";
const nowISO = () => new Date().toISOString();
const clean = (s) => s.replace(/\s+/g, " ").trim();
/** Find the first % **after** a heading index within `forwardChars` */
function percentAfter(text, startIdx, forwardChars = 160) {
...
function sectionAfter(text, re, maxChars = 2000) {
...
function extractSevanaFixed(text) {
    const out = [];
...
function extractSevanaFloating(text) {
    const out = [];
    const m = text.match(/Sevana\s+Housing\s+Loans.*?Floating/i) ||
...
function extractAOCL(text) {
    const out = [];
    const raw = sectionAfter(text, /All\s+Other\s+Consumption\s+Loans/i, 2500);
    const normalized = clean(raw);
    // Secured (LAP)
    const sec = normalized.match(/(\d{1,2}(?:[.,]\d{2})?)\s*%\s*(?:p\.?\s*a\.?)?[^%]{0,120}Secured/i) ||
    ...
    const unsec = normalized.match(/(\d{1,2}(?:[.,]\d{2})?)\s*%\s*(?:p\.?\s*a\.?)?[^%]{0,120}Unsecured/i) ||
...
function extractSpecialPersonalLoans(text) {
    const out = [];
    // Medical Officers
...
function extractSalariedEmployees(text) {
    const out = [];
...
async function scrapeSampath(pdfUrl, outputPath) {
    const buf = Buffer.from(await (await fetch(pdfUrl)).arrayBuffer());
    const pdfData = await (0, pdf_parse_1.default)(buf);
    const text = pdfData.text || "";
    const rows = extractRatesFromText(text);
    await promises_1.default.writeFile(outputPath, JSON.stringify(rows, null, 2), "utf8");
    return rows;
}
/* Optional CLI runner */
...
//# sourceMappingURL=sampath.js.map