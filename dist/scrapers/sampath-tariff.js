...
exports.scrapeSampathTariff = scrapeSampathTariff;
...
function normSpaces(s) { return s.replace(/\u00A0/g, " ").replace(/[ \t]+/g, " ").trim(); }
function normAll(txt) { return txt.split(/\r?\n+/).map(normSpaces).filter(Boolean); }
// ----------- System commands -----------
function execOk(cmd, args, opts) {
    const out = (0, child_process_1.spawnSync)(cmd, args, { encoding: "utf8", ...opts });
    if (out.error)
        throw out.error;
    if (out.status !== 0)
        throw new Error(`${cmd} failed: ${out.stderr || out.stdout}`);
    return out;
}
function pdfToImages(pdfPath, outDir, dpi = 300) {
    const outPrefix = path_1.default.join(outDir, "page");
    execOk("pdftoppm", ["-r", String(dpi), "-png", pdfPath, outPrefix]);
    return fs_1.default.readdirSync(outDir)
        .filter(f => /^page-\d+\.png$/i.test(f))
        .map(f => path_1.default.join(outDir, f))
        .sort((a, b) => Number(a.match(/-(\d+)\.png$/)[1]) - Number(b.match(/-(\d+)\.png$/)[1]));
}
function ocrImageToText(pngPath) {
    const base = pngPath.replace(/\.png$/i, "");
    execOk("tesseract", [
        pngPath, base, "-l", "eng", "--oem", "1", "--psm", "4", // <-- PSM 4 for table OCR!
        "-c", "preserve_interword_spaces=1"
    ]);
    return fs_1.default.readFileSync(base + ".txt", "utf8");
}
/* ---------------- parsing ---------------- */
function parseSampathTariffLines(lines, source) {
    const out = [];
    // Dump OCR for further debugging
    try {
        ensureDir("./work_sampath");
        fs_1.default.writeFileSync("./work_sampath/sampath-tariff-ocr-lines.txt", lines.map((l, idx) => `[${idx}] ${l}`).join("\n"), "utf8");
    ...
    for (let i = 0; i < lines.length; ++i) {
        if (/sevana\s+loans/i.test(lines[i])) {
            ...
            const addBothProducts = (description, amount) => {
                ["Home Loan", "Loan Against Property"].forEach(product => {
                    out.push({
                        bank: "Sampath Bank",
                        product,
...
/* ---------------- main ---------------- */
async function scrapeSampathTariff() {
    const tmpDir = path_1.default.join(os_1.default.tmpdir(), "sampath-tariff");
    ensureDir(tmpDir);
    // Download PDF locally for pdftoppm
    const res = await fetch(PDF_URL, { cache: "no-store" });
    if (!res.ok)
        throw new Error(`HTTP ${res.status} for ${PDF_URL}`);
    const buf = Buffer.from(await res.arrayBuffer());
    const pdfPath = path_1.default.join(tmpDir, "credit_charges.pdf");
    ...
    try {
        fs_1.default.writeFileSync(path_1.default.join(tmpDir, "ocr_output.txt"), combined, "utf8");
    }
    catch { }
    // Normalize → parse
...
exports.default = scrapeSampathTariff;
//# sourceMappingURL=sampath-tariff.js.map