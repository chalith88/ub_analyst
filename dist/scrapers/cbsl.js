...
async function tryGoogleThenFollow(page) {
    // Try Google search → first result; if CAPTCHA/issue, go straight to CBSL.
    try {
        await page.goto(GOOGLE, { waitUntil: "domcontentloaded", timeout: 60000 });
        ...
        if (!href)
            throw new Error("no-search-link");
        await Promise.all([
            page.waitForNavigation({ waitUntil: "domcontentloaded" }),
            firstResult.click(),
        ]);
        ...
        await page.goto(CBSL_REPORT, { waitUntil: "domcontentloaded", timeout: 60000 });
        return page.url();
    }
}
async function scrapeCBSL(opts = {}) {
    ...
    try {
        ...
        const arrivedUrl = opts.preferDirect === "true"
            ? (await page.goto(CBSL_REPORT, { waitUntil: "domcontentloaded", timeout: 60000 }), page.url())
            : await tryGoogleThenFollow(page);
        // 2) Enter date range and click Show
        const from = "2025-01-01";
        const to = lastDayOfCurrentMonth();
        await page.evaluate(({ from, to }) => {
            const f = document.querySelector("#ContentPlaceHolder1_txtFrom");
            const t = document.querySelector("#ContentPlaceHolder1_txtTo");
            if (f) {
                f.value = from;
        ...
        await Promise.all([
            ...
            page.locator("#statTB").waitFor({ state: "visible", timeout: 30000 }),
        ]);
        await page.locator("#statTB tr").nth(6).waitFor({ timeout: 10000 }).catch(() => { });
            ...
            for (let i = 0; i < subHeaders.length; i++) {
                ...
                if (txt === "6 months" || txt === "6months") {
                    ...
                    if (seenSix === 1) {
                        awpr6Index = 2 + i;
                        break;
                    } // shift after [End Week, AWLR]
                }
            }
        }
        if (awpr6Index === -1)
            awpr6Index = 4; // fallback for current layout
        ...
        function pickClosest(rows, anchorTs) {
            let best = rows[0];
        ...
        if (opts.save === "true") {
            ...
            await promises_1.default.writeFile(path_1.default.join(outDir, "cbsl.json"), JSON.stringify(out, null, 2), "utf8");
        }
        return out;
    }
    finally {
