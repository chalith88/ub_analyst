...
exports.scrapeAmanaTariff = scrapeAmanaTariff;
...
async function scrapeAmanaTariff(opts = {}) {
    ...
    for (let p = 1; p <= pdf.numPages; p++) {
        ...
        for (const ln of lines)
            allLines.push({ page: p, text: ln });
    }
    // Dump OCR lines (for traceability)
    const outDir = await ensureOutputDir();
    const ocrPath = path_1.default.join(outDir, "amana-tariff-ocr-lines.txt");
    ...
    if (String(opts.save).toLowerCase() === "true") {
        const jsonOut = path_1.default.join(outDir, "amana-tariff.json");
        await promises_1.default.writeFile(jsonOut, JSON.stringify(rows, null, 2), "utf8");
    }
    return rows;
}
//# sourceMappingURL=amana-tariff.js.map