...
function extractRange(s) {
    const m = s.match(/([0-9]+(?:\.[0-9]+)?)\s*%\s*[-–]\s*([0-9]+(?:\.[0-9]+)?)\s*%/);
    return m
        ? [`${m[1]}%`, `${m[2]}%`]
...
function pushRow(out, base, years) {
    const rows = [];
...
async function scrapeNTB() {
    ...
    // 2) Parse with pdf.js
    const pdf = await (0, pdf_mjs_1.getDocument)({ data: pdfData }).promise;
    ...
    const anchors = [];
    lines.forEach((l, idx) => {
        if (/^Personal Loan$/i.test(l.text))
            anchors.push({ idx, name: "personal" });
        if (/^Housing Loan$/i.test(l.text))
            anchors.push({ idx, name: "housing" });
    });
    const sectionForIdx = (idx) => {
        const prev = anchors.filter((a) => a.idx <= idx).sort((a, b) => b.idx - a.idx);
        return prev.length ? prev[0].name : null;
    };
    const rangeOnlyRe = /([0-9]+(?:\.[0-9]+)?)\s*%\s*[-–]\s*([0-9]+(?:\.[0-9]+)?)\s*%/;
    const seen = new Set();
    for (let i = 0; i < lines.length; i++) {
        ...
        let label = labelRaw;
        if (/^01\s*year\s*fixed/i.test(label))
            label = "01 year fixed";
        else if (/^Up\s*to\s*5\s*years/i.test(label))
            label = "Up to 5 years";
        else if (/^Above\s*5\s*years/i.test(label))
            label = "Above 5 years";
        else if (/^05\s*year\s*fixed/i.test(label))
            label = "05 year fixed";
        const [minR, maxR] = extractRange(rangeStr);
        if (!minR)
            continue;
