...
exports.scrapeSeylan = scrapeSeylan;
...
/* ───────────────────────── small helpers ───────────────────────── */
function parseTenureYears(label) {
...
async function visibleTenureTables(page) {
    const all = await page.locator("table").all();
    const res = [];
    for (const t of all) {
        try {
            const headers = (await t.locator("thead tr th").allTextContents()).map((s) => (0, text_1.clean)(s).toLowerCase());
...
async function pickHLTdIndicesFromSecondRow(table) {
    const headerRows = await table.locator("thead tr").all();
    if (headerRows.length >= 2) {
        const lastRow = headerRows[headerRows.length - 1];
        const labels = (await lastRow.locator("th").allTextContents()).map(text_1.clean);
...
function pickHLTdIndicesByRegex(headerTexts) {
    const lc = headerTexts.map((h) => h.toLowerCase());
    const tenureIdx = lc.findIndex((h) => /tenure|period/.test(h));
    const withCard = (s) => /with.*credit.*internet/.test(s);
    const withoutCard = (s) => /without.*credit.*internet/.test(s);
    const is700k = (s) => /salary/.test(s) && (/700/.test(s) || /700,?000/.test(s));
    const is150to699 = (s) => /salary/.test(s) && (/150/.test(s) || /699/.test(s) || /150,?000/.test(s) || /699,?999/.test(s));
    const isOthers = (s) => /others/.test(s);
    let s700_withIdx = null;
    let s700_withoutIdx = null;
    let sLow_withIdx = null;
    let sLow_withoutIdx = null;
    let others_withIdx = null;
    let others_withoutIdx = null;
    for (let i = 0; i < lc.length; i++) {
        const h = lc[i];
        if (is700k(h) && withCard(h) && s700_withIdx === null)
            s700_withIdx = i;
        if (is700k(h) && withoutCard(h) && s700_withoutIdx === null)
            s700_withoutIdx = i;
        if (is150to699(h) && withCard(h) && sLow_withIdx === null)
            sLow_withIdx = i;
        if (is150to699(h) && withoutCard(h) && sLow_withoutIdx === null)
            sLow_withoutIdx = i;
        if (isOthers(h) && withCard(h) && others_withIdx === null)
            others_withIdx = i;
        if (isOthers(h) && withoutCard(h) && others_withoutIdx === null)
            others_withoutIdx = i;
    }
    const base = tenureIdx >= 0 ? tenureIdx : 0;
    const toTd = (idx) => (idx == null ? null : Math.max(0, idx - base));
    return {
...
async function pickPLTdIndicesFromSecondRow(table) {
    const headerRows = await table.locator("thead tr").all();
    if (headerRows.length >= 2) {
        const lastRow = headerRows[headerRows.length - 1];
        const texts = (await lastRow.locator("th").allTextContents()).map((t) => (0, text_1.clean)(t).toLowerCase());
...
function pickPLTdIndicesByRegex(_headerTexts) {
    return {
        td_t1_with: 1,
        td_t1_without: 2,
        td_t2_with: 3,
...
async function processScholarTableLikeSnippet(table, rows, nowISO, typeLabel, noteText) {
    ...
    const headersLC = (await table.locator("thead tr th").allTextContents()).map((s) => (0, text_1.clean)(s).toLowerCase());
    ...
    let idxSecWith = null;
    let idxSecWithout = null;
    let idxUnsecWith = null;
    let idxUnsecWithout = null;
    const norm = (txt) => (0, text_1.clean)(txt).toLowerCase();
    for (const r of hdrRows) {
        const ths = (await r.locator("th").allTextContents()).map(norm);
        for (let i = 0; i < ths.length; i++) {
            const h = ths[i];
            if (/secured/.test(h) && /with.*credit.*internet/.test(h) && idxSecWith === null)
                idxSecWith = i;
            if (/secured/.test(h) && /without.*credit.*internet/.test(h) && idxSecWithout === null)
                idxSecWithout = i;
            if (/unsecured/.test(h) && /with.*credit.*internet/.test(h) && idxUnsecWith === null)
                idxUnsecWith = i;
            if (/unsecured/.test(h) && /without.*credit.*internet/.test(h) && idxUnsecWithout === null)
                idxUnsecWithout = i;
        }
    }
    ...
    const toTd = (i) => Math.max(1, (i ?? 1) - (tenureIdx >= 0 ? tenureIdx : 0));
    const bodyRows = await table.locator("tbody tr").all();
    for (const tr of bodyRows) {
        const tds = (await tr.locator("td").allTextContents()).map(text_1.clean);
        if (tds.length < 2)
            continue;
        ...
        rows.push(...(0, text_1.fanOutByYears)({
            bank: "Seylan",
...
/* ───────────────────────── main ───────────────────────── */
async function scrapeSeylan(opts) {
    ...
    try {
        ...
        await clickMenuItemById(page, "housingloan");
        {
            ...
            if (tables[0]) {
                ...
                const map = via2.ok
                    ? via2
                    : pickHLTdIndicesByRegex((await t.locator("thead tr th").allTextContents()).map(text_1.clean));
        ...
        await clickMenuItemById(page, "personalloan");
        {
            ...
            if (tables[0]) {
                ...
                const mp = via2.ok
                    ? via2
                    : pickPLTdIndicesByRegex((await t.locator("thead tr th").allTextContents()).map(text_1.clean));
                const tierNotes = "Tier 1 = Professionals & Premium Companies with Salary ≥ 300,000/-. " +
        ...
        await clickMenuItemById(page, "seylanscholarloans");
        {
            ...
            // Table 1 → Fixed
            if (tables[0]) {
                await processScholarTableLikeSnippet(tables[0], rows, now, "Fixed", "Secured (CAT A) and Unsecured (CAT B) — With/Without Credit Card & Internet Banking.");
            }
            // Table 2 → Fixed & Floating
            if (tables.length > 1 && tables[1]) {
                await processScholarTableLikeSnippet(tables[1], rows, now, "Fixed & Floating", "Fixed for 01 year & reviewed every 12 months.");
            }
            await scrollToTop(page);
        }
    }
...
//# sourceMappingURL=seylan.js.map