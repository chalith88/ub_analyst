...
/* ---------- core table scraping ---------- */
async function scrapeTariffTable(block) {
    const out = [];
    ...
    for (const tr of bodyRows) {
        ...
        if (/^Home Loans$/i.test(label)) {
            product = "Home Loan";
            continue;
        }
        if (/^Personal Loans Setting Up Charges$/i.test(label)) {
            product = "Personal Loan";
            continue;
        }
        if (/^Personal Loans Top-up Charges$/i.test(label)) {
            product = "Personal Loan - Top Up";
            continue;
        }
        if (/^Education Loans?/i.test(label)) {
            product = "Education Loan";
            continue;
        }
        // Headings we DON'T want – set to IGNORE so following rows are excluded
        if (/^Short Term Cultivation Loans/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        if (/^Microfinance Loans/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        if (/^Leasing$/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        if (/^Hire Purchase$/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        if (/^Temporary Facility$/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        if (/^Project Financing$/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        if (/^Off-?Shore Banking Centre$/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        if (/^Facilities? Secured By Cash$/i.test(label)) {
            product = "IGNORE";
            continue;
        }
        /* ----- row-level skips ----- */
        // Notes, section numbers, completely empty rows
        if (/^Note\b/i.test(label))
            continue;
        if (/^7\./.test(label))
            continue;
        if (!label && !amount)
        ...
        if (!allowed.includes(product.toLowerCase()))
            continue;
        // Skip "As Above"
        if (amount.toLowerCase() === "as above")
            continue;
        // Also skip any *heading* row that slipped through with empty amount (safety)
        if (!amount && /loans?$/i.test(label))
            continue;
        /* ----- keep row ----- */
        out.push({
...
/* ---------- main ---------- */
async function scrapeCombankTariff() {
    const browser = await playwright_1.chromium.launch({ headless: false, slowMo: 200 });
...
//# sourceMappingURL=combank_tariff.js.map