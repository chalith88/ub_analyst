...
exports.scrapeNSBTariff = scrapeNSBTariff;
...
async function scrapeNSBTariff() {
            ...
            for (let j = 1; j <= 3 && i + j < lines.length; ++j) {
                ...
                if (/retail/i.test(subLine)) {
                    ...
                    if (amountMatch) {
                        ...
                        if (!amount.endsWith("/-"))
                            amount += "/-";
                        ["Home Loan", "Personal Loan", "Education Loan"].forEach((product) => rows.push({
                            bank: BANK,
                            product,
            ...
            for (let j = i + 1; j < i + 10 && j < lines.length; ++j) {
                if (/Service Charges for Personal Loan Including/i.test(cleanLine(lines[j]))) {
                    ...
                    for (let k = j + 1; k < j + 6 && k < lines.length; ++k) {
                        ...
                        if (/Gurantors/i.test(cleanLine(lines[k])))
                            break;
                    }
                    break;
                }
...
*/
exports.default = scrapeNSBTariff;
//# sourceMappingURL=nsb-tariff.js.map