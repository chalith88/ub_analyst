...
function parseTenureRange(label, maxYearsFallback) {
    ...
    if (m)
        return { from: Number(m[1]) + 1, to: Number(m[2]) };
...
async function ensureExpandedAndWaitTable(wrapper, page) {
    ...
    await page.waitForFunction((el) => !!el.querySelector(".extra-content table, .extra-content figure.table table"), wrapper, { timeout: 15000 });
    await page.waitForTimeout(120);
}
async function getTBody(wrapper) {
    return await wrapper.$(".extra-content table tbody, .extra-content figure.table table tbody");
}
async function scrapeBOC(opts = {}) {
    ...
    try {
        browser = await playwright_1.chromium.launch({ headless: !show, slowMo: slow });
        ...
        if (!pane)
            throw new Error("advance-rates pane not found");
        ...
        for (const h3 of headers) {
            ...
            if (title.includes("personal")) {
                ...
                if (!tbody)
                    continue;
                const rows = await tbody.evaluate((tb) => Array.from(tb.querySelectorAll("tr")).map((tr) => {
                    const tds = Array.from(tr.querySelectorAll("td"));
                    const leftHTML = tds[0]?.innerHTML || "";
                    const rightHTML = tds[1]?.innerHTML || "";
                    const scheme = leftHTML
                        .replace(/<br\s*\/?>/gi, "\n")
                        .replace(/<[^>]+>/g, "")
                        .split("\n")
                        .map((s) => s.trim())
            ...
            /* --------------------------- HOUSING LOANS -------------------------- */
                ...
                if (!tbody)
                    continue;
                const rows = await tbody.evaluate((tb) => Array.from(tb.querySelectorAll("tr")).map((tr) => Array.from(tr.querySelectorAll("td")).map((td) => (td.innerHTML || "")
                    .replace(/<br\s*\/?>/gi, "\n")
                    .replace(/<[^>]+>/g, "")
                    .replace(/\s+/g, " ")
                    .trim())));
