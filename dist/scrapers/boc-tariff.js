...
exports.scrapeBocTariff = scrapeBocTariff;
const playwright_1 = require("playwright");
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const BANK = "Bank of Ceylon";
const SEED = "https://www.boc.lk/rates-tariff";
const OUT_DIR = path_1.default.join(process.cwd(), "output");
const OUT_FILE = path_1.default.join(OUT_DIR, "boc.json");
...
async function ensureExpandedAndWaitTable(wrapper, page, log) {
...
const EXCLUDE_SECTION_RE = /(temporary overdrafts?|TOD|leasing|guarantee|stand ?by|credit line letters?|crib|sms alerts|fund transfers?|standing orders?|pay orders?|safe deposit|utility bill|collection accounts|treasury bills?|bonds?|repo|locker)/i;
function clean(s) {
    return (s || "").replace(/\s+/g, " ").replace(/\r?\n|\r/g, " ").trim();
}
function detectProduct(section, desc) {
    const hay = `${section} • ${desc}`;
    if (/housing loans?/i.test(hay) || /BOC Housing Loans/i.test(hay))
        return "Home Loan";
    if (/mortgage\s+over\s+immovable\s+property/i.test(hay)
        || /loan against property/i.test(hay)
        || /property\s*secured/i.test(hay))
        return "LAP";
    if (/\bBOC\s+Personal\s+Loans?\b/i.test(hay)
        || /\bPersonal Loans?\b/i.test(hay)
        || /Pensioners' Loans/i.test(hay))
        return "Personal Loan";
    if (/education|student/i.test(hay))
        return "Education Loan";
    return undefined;
}
function detectFeeCategory(desc) {
    if (/valuation/i.test(desc))
        return "Valuation";
    if (/early\s*settlement|full\s*settlement|part\s*settlement/i.test(desc))
        return "Early Settlement";
    if (/legal/i.test(desc))
        return "Legal";
    if (/penal/i.test(desc))
        return "Penal";
    if (/documentation|processing|service charge|application/i.test(desc))
        return "Processing Fee";
    if (/inspection/i.test(desc))
        return "Other";
    return "Processing Fee";
}
async function scrapeBocTariff(opts = {}) {
    ...
    let rows = [];
    try {
        browser = await playwright_1.chromium.launch({ headless: opts.show !== "true" });
        ...
        for (const h3 of headers) {
            ...
            if (!/credit operations/i.test(productTitle))
                continue;
            ...
            if (!wrapper) {
                log("nav: Credit Operations wrapper not found");
                break;
            }
            await ensureExpandedAndWaitTable(wrapper, page, log);
            opened = true;
            const tableRows = await wrapper.$$eval("table.ck-table-resized tbody tr", (trs) => trs.map(tr => {
            ...
            for (const cells of tableRows) {
                ...
                if (!/\d|%/.test(amt))
                    continue;
                if (EXCLUDE_INTEREST_RE.test(`${desc} ${amt}`))
                    continue;
                // ---- SPECIAL: Split Mortgage over Immovable Property for Personal & Business ----
                if (/Mortgage over Immovable Property/i.test(desc) &&
                ...
                if (/early settlement/i.test(currentSubHeader) &&
                    /BOC Personal Loans.*BOC Housing Loans.*Gov Pensioners.*Loan/i.test(currentSubHeader)) {
                    const fanProducts = ["Home Loan", "LAP", "Personal Loan", "Education Loan"];
        ...
        if (!opened)
            log("nav: Credit Operations not opened (heading not found?)");
    ...
    if (opts.save === "true" || typeof opts.save === "undefined") {
        try {
            await promises_1.default.writeFile(OUT_FILE, JSON.stringify(rows, null, 2), "utf8");
            log(`save: ${OUT_FILE}`);
        }
        catch (e) {
...
//# sourceMappingURL=boc-tariff.js.map