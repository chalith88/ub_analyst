...
exports.scrapeHNB = scrapeHNB;
const playwright_1 = require("playwright");
const dom_1 = require("../utils/dom");
const text_1 = require("../utils/text");
const URL = "https://hnb.lk/interest-rates";
// --- local helpers -----------------------------------------------------------
function extractAwprMargin(s) {
    if (!s)
        return "";
...
function extractHybridOptions(a, b) {
    const set = new Set();
    const addFrom = (s) => {
...
function asPercent(s) {
    ...
    return m ? (0, text_1.clean)(m[0]) : t;
}
// Loose tenure expander for labels like "1-3", "4-5", "7"
function expandYearsLoose(label) {
    ...
    if (rng) {
        ...
        if (!isNaN(a) && !isNaN(b)) {
            if (a > b)
                [a, b] = [b, a];
            for (let i = Math.max(1, a); i <= Math.min(40, b); i++)
                out.push(i);
            return out;
        }
    }
...
async function scrapeHNB(opts) {
    ...
    try {
        ...
        // EDUCATION LOANS
        await (0, dom_1.clickLeftMenu)(page, "Education Loans");
...
// --- HELPERS -----------------------------------------------------------------
async function headingAbove(page, element) {
    ...
    return (0, text_1.clean)(txt);
}
// --- HOME LOANS --------------------------------------------------------------
async function scrapeHomeLoans(page, now) {
    const out = [];
    const source = URL;
    const fixedWith = {};
    const fixedWithout = {};
    ...
    for (const tbl of tables) {
        ...
        const h = (0, text_1.clean)(heading);
        if (!h)
            continue;
        if (/foreign currency/i.test(h))
            continue;
        if (!/home loan/i.test(h) && !/special.*home loan/i.test(h))
            continue;
        ...
        for (const tr of bodyRows) {
            ...
            if (isSpecial) {
                ...
                const years = (0, text_1.expandTenureYears)(tenureCell);
                out.push(...(0, text_1.fanOutByYears)({
                    bank: "HNB",
                    product: "Home Loan",
                    type: "Fixed",
            ...
            const looksHybrid = /(followed by).*(awpr|awplr)/i.test(`${salCellRaw} ${nosalCellRaw}`) ||
            ...
            const years = (0, text_1.expandTenureYears)(tenureCell);
            const rowType = (0, text_1.decideType)(withPct, withoutPct, `${typeCell} ${tenureCell}`);
            const base = {
                bank: "HNB",
                product: "Home Loan",
                type: rowType,
            ...
            const fanned = (0, text_1.fanOutByYears)(base, years);
            out.push(...fanned);
            if (rowType === "Fixed") {
                for (const r of fanned) {
                    if (typeof r.tenureYears === "number" && r.tenureYears <= 10) {
                        if (r.rateWithSalary)
                            fixedWith[r.tenureYears] = r.rateWithSalary;
                        if (r.rateWithoutSalary)
                            fixedWithout[r.tenureYears] = r.rateWithoutSalary;
                    }
                }
            }
        }
...
// --- PERSONAL LOANS ----------------------------------------------------------
async function scrapePersonalLoan(page, now) {
    const out = [];
    const source = URL;
    const fixedWithPL = {};
    const fixedWithoutPL = {};
    const hybridPL = [];
    const tables = await page.locator("table").all();
    for (const tbl of tables) {
        const heading = await headingAbove(page, tbl);
        const h = (0, text_1.clean)(heading);
        if (/roof\s*top\s*solar/i.test(h))
            continue;
        if (!/personal loan/i.test(h))
            continue;
        if (/loan against property|lap/i.test(h))
            continue;
        if (/special/i.test(h))
            continue;
        const headers = (await tbl.locator("thead tr th").allTextContents()).map((s) => (0, text_1.clean)(s).toLowerCase());
        const bodyRows = await tbl.locator("tbody tr").all();
        ...
        for (const tr of bodyRows) {
            ...
            const type = (0, text_1.decideType)(withPct, withoutPct, h);
            const fanned = (0, text_1.fanOutByYears)({
                bank: "HNB",
            ...
            if (type === "Fixed") {
                for (const r of fanned) {
                    if (typeof r.tenureYears === "number" && r.tenureYears <= 10) {
                        if (r.rateWithSalary)
                            fixedWithPL[r.tenureYears] = r.rateWithSalary;
                        if (r.rateWithoutSalary)
                            fixedWithoutPL[r.tenureYears] = r.rateWithoutSalary;
                    }
                }
            }
        }
...
async function scrapePersonalSpecial(page, now) {
    const out = [];
    const source = URL;
    const tables = await page.locator("table").all();
    for (const tbl of tables) {
...
async function scrapeLoanAgainstProperty(page, now) {
    const out = [];
    const source = URL;
    const tables = await page.locator("table").all();
    for (const tbl of tables) {
        const heading = await headingAbove(page, tbl);
        const h = (0, text_1.clean)(heading);
        if (!/(loan against property|^lap$|\blap\b)/i.test(h))
            continue;
        const headers = (await tbl.locator("thead tr th").allTextContents()).map((s) => (0, text_1.clean)(s).toLowerCase());
        const bodyRows = await tbl.locator("tbody tr").all();
        ...
        for (const tr of bodyRows) {
            ...
            let years = (0, text_1.expandTenureYears)(tenureCell);
            if (!years.length)
                years = expandYearsLoose(tenureCell);
...
async function scrapeEducation(page, now) {
    const out = [];
    const source = URL;
    const tables = await page.locator("table").all();
    for (const tbl of tables) {
        ...
        for (const tr of bodyRows) {
            ...
            if (!tds.length)
                continue;
            const tenureCell = tds[(idxTenor >= 0 ? idxTenor : 0)] || "";
            const yearsRaw = (0, text_1.expandTenureYears)(tenureCell);
            const years = yearsRaw.length ? yearsRaw : (() => {
