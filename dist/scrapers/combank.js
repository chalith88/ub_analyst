...
exports.scrapeCombank = scrapeCombank;
...
async function clickTopTabLendingRates(page) {
...
async function openExpandBlock(page, title) {
    const link = page
...
async function getAlignedTenureLabels(table) {
    ...
    for (const tr of headerRows) {
        const ths = (await tr.locator("th").allTextContents()).map((t) => (0, text_1.clean)(t)).filter(Boolean);
        if (ths.length > bestThs.length)
            bestThs = ths;
    }
    ...
    if (labels.length > dataCols)
        labels = labels.slice(0, dataCols);
    if (labels.length < dataCols) {
        const extra = [];
        while (labels.length + extra.length < dataCols)
            extra.push(`Col ${labels.length + extra.length + 1}`);
        labels = labels.concat(extra);
    }
    return labels;
}
/** Read a grid like Home/Personal Loans */
async function scrapeGridTable(block, product, nowISO) {
    const out = [];
    ...
    for (const tr of bodyRows) {
        ...
        for (let j = 0; j < colLabels.length; j++) {
            ...
            if (!rate)
                continue;
            const type = (0, text_1.decideType)(rate, undefined, `${tenureLabel} ${rowLabel}`);
            let years = (0, text_1.expandTenureYears)(tenureLabel);
            if (!years.length) {
...
async function scrapeEducationFromAllOtherAdvances(block, nowISO) {
    const out = [];
    ...
    for (const table of tables) {
        ...
        for (const tr of trs) {
            ...
            const cells = cellsRaw.map(text_1.clean).filter((s) => s.length > 0);
            if (!cells.length)
                continue;
...
/* ───────────────────────── main ───────────────────────── */
async function scrapeCombank(opts) {
...
//# sourceMappingURL=combank.js.map