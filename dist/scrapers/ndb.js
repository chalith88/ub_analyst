...
/* ───────────────────────── helpers ───────────────────────── */
function asRate(cell) {
    const t = (0, text_1.clean)(cell);
    if (!t || t === "-" || t === "–")
        return undefined;
    if (/(awpr|awplr)/i.test(t))
    ...
    const n = Number(m[0]);
    if (!isFinite(n) || n <= 0)
        return undefined; // treat 0.00% as not applicable
    return `${n.toFixed(2)}%`;
}
/** Extract ALL numeric rates from a cell, preserving order (skip 0.00). */
function extractRates(cell) {
    const t = (0, text_1.clean)(cell);
    if (!t)
        return [];
    if (/(awpr|awplr)/i.test(t))
...
async function scrollToAdvancesTable(page) {
    ...
    for (const tbl of tables) {
        const ths = (await tbl.locator("thead th").allTextContents()).map(text_1.clean).join(" ").toLowerCase();
...
async function mapColumns(table) {
    const headTexts = (await table.locator("thead th").allTextContents()).map(text_1.clean);
...
function normalizeProduct(groupOrDesc) {
    const s = groupOrDesc.toLowerCase();
    if (/home\s*loan/.test(s) || /housing/.test(s) || /mortgage/.test(s))
        return "Home Loan";
    if (/personal\s*loan/.test(s))
        return "Personal Loan";
    if (/education/.test(s) || /student/.test(s) || /scholar/.test(s))
        return "Education Loan";
    return groupOrDesc;
}
function detectType(desc) {
    const d = desc.toLowerCase();
    if (/variable|floating|awpr|awplr/.test(d))
        return "Floating";
    return "Fixed";
}
/* ───────────────────────── parser ───────────────────────── */
async function parseAdvancesTable(table, nowISO) {
    const out = [];
    ...
    for (const tr of trs) {
        ...
        const cells = tds.map(text_1.clean);
        if (!cells.length)
            continue;
        ...
        let notesBase;
        if (isPersonal) {
            if (/general personal loan/i.test(descClean))
                notesBase = "General personal loan";
            else if (/special rate\s*for\s*doctors/i.test(descClean))
                notesBase = "Special rate for doctors";
            else if (currentGroup && (0, text_1.clean)(currentGroup) !== descClean)
                notesBase = descClean;
        }
        else if (currentGroup && (0, text_1.clean)(currentGroup) !== descClean) {
            notesBase = descClean;
